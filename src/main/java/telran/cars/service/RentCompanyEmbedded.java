...
public class RentCompanyEmbedded extends AbstractRentCompany implements Persistable {
	...
	private static final long serialVersionUID = 5294240813407615069L;

	private static final int GOOD_THREASHOLD = 10;
	private static final int BAD_THREASHOLD = 30;
	private static final int REMOVE_THREASHOLD = 60;
	...
	public CarsReturnCode addCar(Car car) {
		...
		try {
			if (car == null)
				return CarsReturnCode.WRONG_DATA;
	...
	public CarsReturnCode rentCar(String regNumber, long licenseId, LocalDate rentDate, int rentDays) {
		...
		try {
			...
			addToRecors(record);
			addToCarRecors(record);
			addToDriverRecors(record);
			car.setInUse(true);
			return OK;
		} finally {
			lockRentCar(false);
	...
	private void addToDriverRecors(RentRecord record) {
	...
	private void addToCarRecors(RentRecord record) {
	...
	private void addToRecors(RentRecord record) {
	...
	@Override
	public RemovedCarData returnCar(String regNumber, long licendeId, LocalDate returnDate, int damagesPercent,
			int tankPercent) {
		lockRentCar(true);
		try {
			if (regNumber == null || licendeId < 1 || returnDate == null || damagesPercent < 0 || tankPercent < 0)
				return null;
			RentRecord record = getRentRecord(regNumber, licendeId);
			if (record == null)
				return null;
			updateRecord(record, returnDate, damagesPercent, tankPercent);
			Car car = getCar(regNumber);
			updateCar(car, damagesPercent);
			return car.isFlagRemoved() || damagesPercent >= REMOVE_THREASHOLD ? actualCarRemove(car) : null;
		} finally {
			lockRentCar(false);
		}
	}
	...
	private void updateCar(Car car, int damagesPercent) {
		car.setInUse(false);
		if (damagesPercent >= BAD_THREASHOLD)
			car.setState(State.BAD);
		else if (damagesPercent >= GOOD_THREASHOLD)
	...
	private RentRecord getRentRecord(String regNumber, long licendeId) {
		return driverRecords.get(licendeId).stream()
	...
	public List<String> getMostPopularCarModels(LocalDate dateFrom, LocalDate dateTo, int ageFrom, int ageTo) {
		lockGetMostPopularCarModels(true);
		try {
			...
			lockGetMostPopularCarModels(false);
		}
	}
	...
	public List<String> getMostProfitableCarModels(LocalDate dateFrom, LocalDate dateTo) {
		lockGetMostProfitableCarModels(true);
		try {
			List<String> res = new ArrayList<>();
			List<RentRecord> list = getRentRecordsAtDates(dateFrom, dateTo);
			if (list.isEmpty())
			...
			lockGetMostProfitableCarModels(false);
		}
	}

	@Override
	public List<Driver> getMostActiveDrivers() {
		lockGetMostActiveDrivers(true);
		try {
			...
			lockGetMostActiveDrivers(false);
		}
	}
	...
	public List<RentRecord> getRentRecordsAtDates(LocalDate from, LocalDate to) {
		lockGetRentRecordsAtDates(true);
		try {
			...
			lockGetRentRecordsAtDates(false);
		}
	}
